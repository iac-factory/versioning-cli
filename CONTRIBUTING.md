...
5. Make the module available, publicly.
    ```bash
    GOPROXY=proxy.golang.org go list -mutex "versioning-cli/example@v$(head VERSION)"
    ```
...
- Example: https://dev.go.dev/versioning-cli/example

Upon successful request, a message should be displayed:

> _We're still working on “versioning-cli/example”. Check back in a few minutes!_
